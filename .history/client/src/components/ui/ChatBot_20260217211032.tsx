    ...
    <>
      <div className="fixed bottom-6 right-6 md:bottom-8 md:right-8 z-[100]">
        <Button 
          ...
          className="h-12 w-12 md:h-20 md:w-20 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.3)] bg-primary text-primary-foreground hover:scale-105 transition-all duration-300 border-2 border-white/20 p-0 overflow-hidden group"
        >
          {isOpen ? (
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-4 md:bottom-32 md:right-8 w-[calc(100vw-32px)] sm:w-[280px] md:w-[350px] h-[400px] md:h-[480px] glass-card z-[100] flex flex-col overflow-hidden rounded-3xl border border-white/20 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] bg-white/95 dark:bg-black/95 backdrop-blur-3xl"
          >
            <div className="p-6 md:p-8 bg-primary text-primary-foreground flex justify-between items-center relative overflow-hidden">
              ...
              <div className="flex items-center gap-4 relative z-10">
                ...
                <div>
                  <div className="font-display font-black text-sm md:text-md uppercase tracking-[0.2em]">Professional Liaison</div>
                  <div className="text-[6px] font-bold opacity-80 uppercase tracking-widest flex items-center gap-2">
              ...
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[85%] p-5 md:p-6 text-sm md:text-base leading-relaxed ${
                    m.role === "user" 
                      ? "bg-primary text-primary-foreground rounded-[1rem] rounded-tr-none shadow-xl" 
                      : "bg-secondary/60 text-foreground rounded-[1rem] rounded-tl-none border border-border/50 shadow-sm font-medium"
                  }`}>
                    {m.content}
                  </div>
                </div>
