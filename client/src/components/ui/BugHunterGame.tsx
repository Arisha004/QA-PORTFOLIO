  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <div className="relative w-full max-w-6xl bg-white border border-black/10 shadow-[0_100px_200px_-50px_rgba(0,0,0,0.8)] overflow-hidden rounded-[3rem]">
            <div className="absolute top-0 left-0 w-full h-[3px] bg-gradient-to-r from-transparent via-primary to-transparent opacity-50" />
            
            <div className="p-8 md:p-12 border-b border-black/5 flex justify-between items-center bg-gray-50 relative">
            ...
            <div className="relative aspect-[16/9] md:aspect-[21/9] bg-white overflow-hidden group">
              {gameState === "idle" && (
              ...
              {gameState === "finished" && (
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-12 bg-white/95 z-30 backdrop-blur-xl">
                  ...
                  <Button onClick={startGame} className="rounded-2xl px-20 h-24 font-black text-2xl tracking-[0.3em] uppercase bg-black text-white hover:bg-primary transition-all shadow-2xl">
                    RE-INITIALIZE
                  </Button>
                </div>
              )}
