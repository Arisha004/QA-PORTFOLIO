...
export function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled || isMobileMenuOpen ? "bg-black/95 backdrop-blur-md border-b border-white/20 py-4" : "py-6 md:py-8"}`}>
      ...
      {/* Mobile Menu */}
      <div className={`lg:hidden absolute top-full left-0 w-full bg-black border-b border-white/10 transition-all duration-300 overflow-hidden ${isMobileMenuOpen ? "max-h-screen opacity-100 py-8" : "max-h-0 opacity-0"}`}>
        <div className="container-custom flex flex-col gap-6">
          ...
          <Button className="rounded-none border-primary bg-primary text-black font-black text-[10px] tracking-widest uppercase h-12 w-full">
            CONTACT ME
          </Button>
        </div>
      </div>
